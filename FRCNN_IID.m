function FRCNN_IID(imPath, initImgPath, resDir)
%% GC
...
% feat_G = getFRCNNFeatures(I, boxes);
% feat_L = getFRCNNFeatures(I, boxes);
% feat_L = getFRCNNFeatures_pool5(I, boxes);
...
% nweightMap_s = getNormalConstraintMatrix(normGCmap, sig_n);
...
A = 4 * WRC + 1 * mask * (spI - LLEGRID) + 1 * mask2 * (spI - LLENORMAL) + 1 * L_S + 0.025 * WSC;  % <-- FOR RCNN NET
