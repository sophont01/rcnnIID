function anyImageRCNN_clusterIID_4(imPath, resDir, sig_c, sig_i)
...
[numClusters, clusterLabels, Rp1, Rp2] = getIIDClusters(imPath,15);
clusterLabels = imresize(clusterLabels,[ size(I,1) size(I,2) ]); % NOTE : do before resize
Clus = reshape(clusterLabels, [size(I,1) size(I,2)]);

I = imresize(I,[ hn wn ]);
Clus = imresize(Clus,[ hn wn ]);
...
C = getChrom(S);
[consVec_retinex, ~] = getContinuousConstraintMatrix(C, sig_c, sig_i, S);


% Compute cluster reflectance constraint 
[SL,Sa,Sb] = rgb2lab(S);
% consVec_clusRef = getClusteringConstraintsRef(S,Clus);
consVec_clusRef = getClusteringConstraintsRef_1(C,S,Clus);
...
lambdaP = 4 ;
A = lambdaP * (WRC + WRClus) + 1 * mask * (spI - LLEGRID) + 1 * mask2 * (spI - LLERP) + 1 * L_S + 0.025 * WSC; % + 2 * WRClus;
% A = 1 * mask * (spI - LLEGRID) + 1 * mask2 * (spI - LLERP) + 1 * L_S; % + 0.025 * WSC;
b = lambdaP * (consVec_retinex +  consVec_clusRef);
