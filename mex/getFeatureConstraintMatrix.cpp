...
{
	int featureDim = mxGetPr(prhs[2])[0];
	int h = mxGetM(prhs[0]);
	int w = mxGetN(prhs[0])/featureDim;
	int i, j, k;
	int nx[] = {0, 0, 1, -1, -1, 1, 1, -1};
	int ny[] = {1, -1, 0, 0, -1, 1, -1, 1};
	double cp[featureDim], cq[featureDim];
	double *feature = mxGetPr(prhs[0]);
	double dist;
